...
public class Queue {
    ...
    public void enqueue(Node y) { //Parameter is a node because we will be a new node being enqueued in the queue
        Node node = new Node(y.getName(), y.getNext()); //I created a new node in order to make sure there were not any extra pointers pointing to something other than the next node in the queue
        if (head == null) { //If the queue was empty, then the if loop will make it so that the incoming node to the queue will become both the head and the tail
            tail = node; //Setting the incoming node to the tail
            head = tail;  //Setting the head to the tail
        }
        else { //The else statement is for if there are already nodes in the queue
            tail.setNode(node); //Opposite of the stack because in the stack we are bringining in a new node which must point to the old head, but in the queue the tail is already there and needs to be pointed to the incoming node
            tail = node; //Setting the incoming node to the tail
        }
    }
